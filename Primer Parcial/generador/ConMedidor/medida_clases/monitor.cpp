...
long Monitor::obtener_memoria() {
    // macOS: usar mach_task_basic_info para obtener RSS en bytes
#if defined(__APPLE__) && defined(__MACH__)
    ...
    return static_cast<long>(info.resident_size / 1024); // a KB
#else
    // Linux: leer /proc/self/statm para obtener páginas residentes y convertir a KB
