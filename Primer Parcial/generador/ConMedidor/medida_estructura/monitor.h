...
public:
    ...
    // Obtiene el consumo de memoria actual (RSS) en KB (multiplataforma)
    long obtener_memoria() {
        // macOS: usar mach_task_basic_info
#if defined(__APPLE__) && defined(__MACH__)
        ...
        return static_cast<long>(info.resident_size / 1024); // KB
#else
        ...
        fclose(file);
        long page_size_kb = sysconf(_SC_PAGESIZE) / 1024; // Convertir a KB
